...
//			EventQueue.invokeLater(this);
    ...
    private Point2D componentPaint(Graphics g) {
    	...
    	//-----------------------
    	g2d.setStroke(GrappaStyle.defaultStroke);//デフォルトの線の属性を与える
    	...
    	synchronized(graph) {
    		...
    		if(grappaNexus != null) {
    			...
    			if(grappaNexus.style.filled || grappaNexus.image != null) {
    				// 塗りつぶしありならその色に設定してgrappaNexusの形を描画
    				if(grappaNexus.style.filled) {
    					//追加、グラデーションを描画する場合
    					if(grappaNexus.gradient != null){
    						g2d.setPaint(grappaNexus.gradient);
    					...
    					//----------------------------------
    					else if (grappaNexus.fillcolor != null) {
    		...
    		if(subg != subgraph) {
    			...
    			if(grappaNexus.style.filled) {
    				if (grappaNexus.fillcolor != null) {
    					//bkgdColor = grappaNexus.fillcolor[0];
    				...
    				for( Edge edge : edges ) {
    					...
    					if((grappaNexus = edge.grappaNexus) != null && edge.visible && !grappaNexus.style.invis && clipper.intersects(grappaNexus.rawBounds2D())) {
    						...
    						if((edge.highlight&DELETION_MASK) == DELETION_MASK) {
    							...
    							//エッジの始点を描画
    							Attribute at = edge.getLocalAttribute(ARROWHEAD_ATTR);
    							if(at != null){
    								GrappaPoint[] pts = gl.getGrappaPoints() ;
    								String s = at.getStringValue();
    								if(s.equals("dot"))
    									g2d.fillOval((int)(pts[0].x-5), (int)(pts[0].y-5), 10, 10);
    								else if(s.equals("odot")){
    									g2d.setPaint(Color.white);
    									g2d.fillOval((int)(pts[0].x-4), (int)(pts[0].y-4), 8, 8);
    									g2d.setPaint(Color.black);
    									g2d.drawOval((int)(pts[0].x-5), (int)(pts[0].y-5), 10, 10);

    								}
    								g2d.setPaint(grappaNexus.color);
    							}
    							//エッジの始点を描画
    							at = edge.getLocalAttribute(ARROWTAIL_ATTR);
    							if(at != null){
    								GrappaPoint[] pts = gl.getGrappaPoints() ;
    								String s = at.getStringValue();
    								int l = pts.length-1;
    								if(s.equals("dot"))
    									g2d.fillOval((int)(pts[l].x-5), (int)(pts[l].y-5), 10, 10);
    								else if(s.equals("odot")){
    									g2d.setPaint(Color.white);
    									g2d.fillOval((int)(pts[l].x-4), (int)(pts[l].y-4), 8, 8);
    									g2d.setPaint(Color.black);
    									g2d.drawOval((int)(pts[l].x-5), (int)(pts[l].y-5), 10, 10);
