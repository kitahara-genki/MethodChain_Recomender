	...
	Cloneable
{
     ...
     * odotを表す値
     */
    public final static int STYLE_ODOT			= 19;
    /**
     * odotを表す値
     */
    public final static int STYLE_DOT			= 20;
	...
	// 追加部分--------------------------------------------------------
	styleTypes.put("striped",new Integer(STYLE_FILLED));
	styleTypes.put("wedged",new Integer(STYLE_FILLED));
	styleTypes.put("dot",new Integer(STYLE_DOT));
	styleTypes.put("odot",new Integer(STYLE_ODOT));
	//----------------------------------------------------------------
   ...
   //線の状態
	...
	while(i<len) {
		...
		option_string = style.substring(offset,i);//styleの余計な文字のない文字列だけを取る
		if(in_parens) {
		    if(last_option == -1) {
		    ...
		    //線のスタイルをoptionの値にする。dashは破線の間隔と長さ、invisは透明かどうか
		    last_option = -1;
		    switch(option) {
		    case STYLE_SOLID:
			line_style = option;
