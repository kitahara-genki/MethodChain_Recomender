...
import java_cup.geruntime.Symbol;
...
public class Lexer
{
	...
	if(retreated) {
	    ...
	    old_char = next_char;
	    next_char = next_char2;
	    //next_charがエラーならその後の文字は読み込まず、next_char2もエラーにする
	    if (next_char == EOF_CHAR) {
	    	next_char2 = EOF_CHAR;
	    } else {
	    //next_charがエラーでないなら1文字読み込む、advanceが呼ばれたらまずここで1文字読み込む
	 ...
	 * \→改行 or \復帰文字の場合、1文字読み込む
	 */
	if (next_char == '\\' && (next_char2 == '\n' || next_char2 == '\r')) {
	    next_char = next_char2;
	    next_char2 = inReader.read();
	    //読み込んだ時、1文字が改行で、2文字目が復帰文字だったらさらに1文字読み込む
	    if(next_char == '\r' && next_char2 == '\n') {
		next_char = next_char2;
		next_char2 = inReader.read();
	    }
	    //更に1文字読み込む、2or3文字読み込んだのでnext_charは改行文字ではない
	    next_char = next_char2;
	    next_char2 = inReader.read();
	}
    ...
    /**
     * next_char, next_char2の値を1文字前の値に戻す。その時に、retreatedの値をtrueにする。
     * その後advance関数が呼ばれたときはnext_charとnext_char2の値は元に戻る。
     */
     ...
     * chがchar_symbolsに登録されていればそのハッシュの値を返す。ハッシュにない文字だったら-1を返す。
     * 文字は:や{などである
     * 
     * @param ch the character in question. chはnext_char
     */
     ...
     * " "で囲まれた文字列を読み込んで取り出す。取り出された文字列はATOMとして扱われる。
     */
    private Symbol do_quote_string() throws IOException {
	String result_str;

	// at this point we have lookahead of a double quote -- swallow that
	// これが呼ばれるときはnext_charが '"' の時なので1文字進める
	advance();

	synchronized(cmnstrbuf) {
	    cmnstrbuf.delete(0,cmnstrbuf.length()); // faster than cmnstrbuf.setLength(0)!全部の文字を消す
		...
		// 1文字読んでStringBufferに足す、これはATOMとして扱われる
	...
	// 現在のnext_charは '"' なので1文字読み込む
	advance();
	haveId = true;
	//<値を表す数値、実際の値(""で囲まれた文字列)>を要素としたシンボルを返す
	return new Symbol(Symbols.ATOM, result_str);
    }
    /**
     * do_quote_stringのように数字をまとめてATOMとしてシンボルにして返す
     * @return
     * @throws IOException
     */
    private Symbol do_number_string() throws IOException {
    	String result_str;
	...
	// キーワードに登録された単語ならばその値を使ってシンボルに登録
	if (keyword_num != null) {
	    haveId = false;
     ...
     * debug_next_token().実際に次のトークンを取ってくる関数。next_token()から呼び出されりゅ
     * advance関数を用いて
     */
	...
	for (;;) {
		...
		//next_charが次のような時はadvanceで1文字読み込む
	    ...
	    // next_char, next_char2より、edgeと判断出来る時、2文字次を読みこんでedgeのシンボルを返す
	    if (next_char == '-') {
	    	if (next_char2 == '>') {//有効グラフ
	    		advance();
	    		advance();
	    ...
	    // ハッシュテーブルから値を取ってくる。next_charが ';' や '{' などのときは-1が返ってくる
	    sym_num = find_single_char(next_char);
	    //------------------------------------------------------------------------
	    // next_charの値がchar_symbolsに登録されているものである時
	    //------------------------------------------------------------------------
	    if (sym_num != -1) {
	    	//サブグラフの時
	    	...
	    	// サブグラフでないならもう1文字読み込み、sym_numの値を返す
	    	advance();
	    	haveId = false;
	    	return new Symbol(sym_num);
	    }
	    //------------------------------------------------------------------------
	    // next_charの値がchar_symbolsに登録されていないものである時
	    //------------------------------------------------------------------------
	    // look for quoted string
	    // next_charが '"' なら " "で囲まれた文字列を読み込んで取り出す
	    if (next_char == '"') {
		...
		//数字が来たら
	    //これを作った
	    if(old_char == '=' && 48 <= next_char && next_char <= 57){
	    	if(!(MyMenuBar.getOld())){
	    		return do_number_string();
	    	}
	    }
	    ...
	    if (next_char == '/' && (next_char2 == '*' || next_char2 == '/')) {
	    	//System.out.println("swalloooooooooooooooooooooow");
	    ...
	    if (id_char(next_char)) {
	    	//System.out.println("keyworddddddddddddddddddddd");
		return do_id();
	    }
